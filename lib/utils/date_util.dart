...
const int day = 24 * 3600 * 1000;
const int month = 30 * 24 * 3600 * 1000;
...
class DateUtil {
  ///
  ///
  static TimeDelta dateDeltaText(DateTime date) {
    DateTime current = DateTime.now();
    int delta = current.difference(date).inDays;
    if (delta == 0) {
      return TimeDelta(time: 0, text: '今天');
    }
    if (delta == 1) {
      return TimeDelta(time: 0, text: '昨天');
    }
    if (delta == 2) {
      return TimeDelta(time: 0, text: '前天');
    }
    return TimeDelta(time: delta, text: '天前');
  }

  ///
  ///
  static TimeDelta timeDeltaText(DateTime time) {
    int currentMillis = DateTime.now().millisecondsSinceEpoch;
    int timeMillis = time.millisecondsSinceEpoch;
    int delta = currentMillis - timeMillis;
    if (delta <= minute) {
      return TimeDelta(
        time: 1,
        text: '分钟内',
  ...
  static DateTime parse(
    String dateTimeStr, {
    String pattern = DateFormats.y_mo_d,
    bool isUtc = false,
  }) {
    return DateFormat(pattern).parse(dateTimeStr, isUtc);
  }
