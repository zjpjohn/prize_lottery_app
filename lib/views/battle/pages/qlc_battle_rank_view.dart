...
class QlcBattleRankView extends StatelessWidget {
  ///
  ///
  const QlcBattleRankView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: RefreshWidget<QlcBattleMasterController>(
        init: QlcBattleMasterController(),
        builder: (controller) {
  ...
  Widget _buildBattleMaster(
      QlcBattleMasterController controller, int index, bool border) {
    MasterBattleRank<QlcMasterMulRank> rank = controller.ranks[index];
    return BattleRankEntryWidget(
      ...
      data: MulMasterRank(
        ...
        achieves: [
          AchieveInfo(name: '围码', count: rank.masterRank.red22.count),
          AchieveInfo(
              name: '杀码',
              count: rank.masterRank.kill3.count,
              color: TagColor.blue),
        ],
      ),
      onTap: () {
        QlcBattleController().addBattle(
