...
class DltFullCensusController extends AbsFeeRequestController {
  ///
  ///全量统计级别
  int _level = 100;

  ///
  Map<int, DltChartCensusDo> censusMap = {};
  ...
  void getLevelFullCensus({bool showLoading = false}) {
    ...
    LottoCensusRepository.dltLevelFullV1(level).then((value) {
      Future.delayed(const Duration(milliseconds: 200), () {
        feeBrowse = value.feeRequired;
        period = value.period;
        showSuccess(value, success: () {
          if (!feeBrowse && value.data != null) {
            censusMap[level] = DltChartCensusDo(value.data!);
            census = censusMap[level]!;
          }
        });
      });
