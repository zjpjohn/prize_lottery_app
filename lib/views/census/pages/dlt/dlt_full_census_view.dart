...
class DltFullCensusView extends StatelessWidget {
  ///
  ///
  const DltFullCensusView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        ...
        child: FeeCensusRequestWidget<DltFullCensusController>(
          title: '查看大乐透整体趋势分析',
          adsName: '整体预测分析',
  ...
  Widget _buildTitleHeader(DltFullCensusController controller) {
    return Container(
  ...
  Widget _buildDescription() {
    return Container(
      ...
      child: Text(
        '大乐透整体分析通过对上一期预测推荐排名前100、200、400、600、800以及1000的专家'
        '本期推荐方案进行分级分指标综合计算分析，计算出本期预测推荐在不同指标维度下整体推荐和杀码热度趋势。'
        '整体统计趋势非开奖日19、22点更新，开奖日当天15点开始更新，每小时更新一次直至19点结束。',
        style: TextStyle(
          height: 1.3,
          fontSize: 15.sp,
          color: Colors.black,
  ...
  Widget _buildLevelFilter(DltFullCensusController controller) {
    List<MapEntry<int, String>> entries = fullLevels.entries.toList();
    return Container(
  ...
  Widget _buildRedTrendChart(DltFullCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildBlueTrendChart(DltFullCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildNoticeView() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1、整体趋势分析指标包含三胆、10码、20码、杀三码、杀六码、蓝球双胆、蓝球六码以及蓝球杀码等，并进行选号热度统计分析。',
            style: TextStyle(
              fontSize: 12.sp,
              color: Colors.black45,
            ),
          ...
          Text(
            '3、关于整体趋势分析指标使用，需要您在使用过程中结合自身选号经验多观察、多总结，相信一定能为您带来意想不到的收获。',
            style: TextStyle(
              fontSize: 12.sp,
              color: Colors.black45,
            ),
