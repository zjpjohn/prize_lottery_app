...
class DltVipCensusView extends StatelessWidget {
  ///
  ///
  const DltVipCensusView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        ...
        child: FeeCensusRequestWidget<DltVipCensusController>(
          title: '查看大乐透综合趋势分析',
          adsName: '综合预测分析',
  ...
  Widget _buildTitleHeader(DltVipCensusController controller) {
    return Container(
  ...
  Widget _buildDescription() {
    return Container(
      ...
      child: Text(
        '大乐透综合分析通过对上一期预测推荐排名前10、20、50、100以及150的付费专家本期推荐方案'
        '进行分级分指标汇总计算分析，计算出本期预测推荐在不同指标维度下综合推荐和杀码热度趋势。'
  ...
  Widget _buildLevelFilter(DltVipCensusController controller) {
  ...
  Widget _buildRedTrendChart(DltVipCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildBlueTrendChart(DltVipCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildNoticeView() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1、综合趋势分析指标包含三胆、10码、20码、杀三码、杀六码、蓝球双胆、蓝球六码以及蓝球杀码等，并进行选号热度统计分析。',
            style: TextStyle(
              fontSize: 12.sp,
              color: Colors.black45,
            ),
          ...
          Text(
            '3、关于综合趋势分析指标使用，需要您在使用过程中结合自身选号经验多观察、多总结，相信一定能为您带来意想不到的收获。',
            style: TextStyle(
              fontSize: 12.sp,
              color: Colors.black45,
            ),
