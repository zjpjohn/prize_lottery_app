...
class Fc3dFullCensusView extends StatelessWidget {
  ///
  ///
  const Fc3dFullCensusView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
          ...
          child: FeeCensusRequestWidget<Fc3dFullCensusController>(
            title: '查看最新号码热度整体趋势',
            adsName: '整体预测分析',
  ...
  Widget _buildTitleHeader(Fc3dFullCensusController controller) {
    return Container(
  ...
  Widget _buildDescription() {
    return Container(
      ...
      child: Text(
        '福彩3D整体分析通过对上一期预测推荐分类排名前10、20、50、100以及150的专家'
        '本期推荐方案进行分级分指标汇总计算分析，计算出本期预测推荐在不同指标维度下整体推荐和杀码热度趋势。'
  ...
  Widget _buildLevelFilter(Fc3dFullCensusController controller) {
  ...
  Widget _buildTrendChart(Fc3dFullCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildLegendView(Fc3dFullCensusController controller) {
    return Padding(
  ...
  Widget _chartLegend(ChartLegend legend, Fc3dFullCensusController controller) {
