...
class Pl3VipCensusView extends StatelessWidget {
  ///
  ///
  const Pl3VipCensusView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        ...
        child: FeeCensusRequestWidget<Pl3VipCensusController>(
          title: '查看最新号码热度综合趋势',
          adsName: '综合预测分析',
  ...
  Widget _buildTitleHeader(Pl3VipCensusController controller) {
    return Container(
  ...
  Widget _buildDescription() {
    return Container(
      ...
      child: Text(
        '排列三综合分析通过对上一期预测推荐排名前10、20、50、100以及150的付费专家本期推荐方案'
        '进行分级分指标汇总计算分析，计算出本期预测推荐不同指标下综合综合推荐和杀码热度趋势。'
  ...
  Widget _buildLevelFilter(Pl3VipCensusController controller) {
  ...
  Widget _buildTrendChart(Pl3VipCensusController controller) {
    List<Widget> items = [];
  ...
  Widget _buildLegendView(Pl3VipCensusController controller) {
    return Padding(
  ...
  Widget _chartLegend(ChartLegend legend, Pl3VipCensusController controller) {
