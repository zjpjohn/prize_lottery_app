...
import 'package:prize_lottery_app/views/forecast/controller/pl3_forecast_controller.dart';
...
class Pl3ForecastView extends StatefulWidget {
  const Pl3ForecastView({super.key});

  @override
  State<Pl3ForecastView> createState() => _Pl3ForecastViewState();
}

class _Pl3ForecastViewState extends State<Pl3ForecastView> {
  @override
  Widget build(BuildContext context) {
    return FeeShareRequestWidget<Pl3ForecastController>(
      global: false,
      background: Colors.white,
      init: Pl3ForecastController(),
      adsBg: ResourceStore().resource(R.pl3ForecastBg),
      adsTitle: '查看排列三预测详情',
      adsName: '预测方案',
      title: '排列三方案',
      share: (controller) {
      ...
      battle: (controller) {
        Pl3BattleController().addAndRoute(controller.masterId);
      },
      featureTap: (controller) {
        Get.toNamed(
          '/pl3/master/feature/${controller.masterId}',
        );
      },
      builder: (controller) {
  ...
  Widget _buildForecastCard(
    Pl3ForecastController controller,
    Pl3ForecastInfo forecast,
  ) {
    return Container(
  ...
  Widget _buildTitle(Pl3ForecastInfo forecast) {
    return Container(
  ...
  void _showSharePoster(Pl3ForecastController controller) {
    Constants.shareBottomSheet(
      content: ForecastPosterView(
        posterKey: controller.posterKey,
        title: '排列三第${controller.forecast.period}期',
        forecasts: {
          '杀一码': ForecastItem(
