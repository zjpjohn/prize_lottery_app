...
class DiagramsTableController extends AbsRequestController {
  ///开奖类型
  late String type;

  ///速查表信息
  late DiagramsTable diagramsTable;
  Map<String, DiagramsTable> cache = {};
  ...
  set period(String? period) {
    ...
    if (cache[_period] != null) {
      diagramsTable = cache[_period]!;
    }
    update();
    if (cache[_period] == null) {
      EasyLoading.show(status: '加载中');
      LotteryInfoRepository.diagramTable(type: type, period: _period)
          .then((value) {
        diagramsTable = value;
        cache[_period!] = diagramsTable;
        update();
      }).catchError((error) {
        showError(error);
      }).whenComplete(() {
  ...
  List<Future<void>> asyncTasks() {
    return [
      ...
      LotteryInfoRepository.diagramTable(type: type).then((value) {
        diagramsTable = value;
        cache[diagramsTable.current.period] = diagramsTable;
      })
    ];
  }
