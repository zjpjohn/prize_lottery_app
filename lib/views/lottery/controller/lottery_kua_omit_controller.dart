...
class LotteryKuaOmitController extends AbsRequestController {
  ///
  /// 彩票类型
  final String type;
  ...
  ///
  late N3KuaCensus census;

  LotteryKuaOmitController(this.type);
  ...
  List<Future<void>> asyncTasks() {
    return [
      ...
      LotteryInfoRepository.kuaOmits(type, _size).then((value) {
        omits
          ..clear()
          ..addAll(value)
          ..sort((e1, e2) => e1.period.compareTo(e2.period));
        if (omits.isNotEmpty) {
          census = N3KuaCensus(omits);
        }
      }),
    ];
  }
