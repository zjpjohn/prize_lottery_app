...
class LotteryTrendController extends AbsRequestController {
  ///彩种类型
  late String type;
  ...
  ///遗漏统计数据
  List<TrendOmit> omits = [];
  late TrendCensus census;

  LotteryTrendController(this.type);
  ...
  List<Future<void>> asyncTasks() {
    return [
      ...
      LotteryInfoRepository.trendOmits(type, _size).then((value) {
        omits
          ..clear()
          ..addAll(value)
          ..sort((e1, e2) => e1.period.compareTo(e2.period));
        if (omits.isNotEmpty) {
          census = TrendCensus(omits);
        }
      }),
    ];
  }
