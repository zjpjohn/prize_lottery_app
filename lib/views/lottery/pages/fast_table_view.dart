...
class FastTableView extends StatelessWidget {
  ///
  ///
  const FastTableView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        color: Colors.white,
        child: RequestWidget<FastTableController>(
          builder: (controller) {
            return Stack(
              children: [
                ScrollConfiguration(
  ...
  Widget _buildPeriodView(FastTableController controller) {
    return Container(
      width: 34.w,
  ...
  Widget _buildLayoutContent(FastTableController controller) {
  ...
  Widget _buildPosterWidget(FastTableController controller) {
    return Container(
      color: Colors.white,
      width: Get.width,
  ...
  Widget _buildHeaderView(FastTableController controller) {
    return Container(
      ...
      child: Column(
        children: [
          Text(
            '第${controller.fastTable.current.period}期${lotteryZhCns[controller.type]!}速查表',
            style: TextStyle(
              color: Colors.black,
              fontSize: 18.sp,
  ...
  Widget _buildTableDescription() {
    return Container(
      ...
      child: Text(
        '速查表是专门为选三型彩票提供的一种选号参考工具。在上一期开奖号码中找到临近数字的连线，'
        '以其中一个数字为轴，找到下一期三个临近数字的连线，即有可能为当期的开奖号码。其中连线可能为折线或直线，需根据自身选号经验做出判断。'
        '速查表数据每天下午17:00左右试机号更新完成后会显示最新一期的速查表。用户可长按选号图表，将本图表保存至本地更方便地查看。',
        style: TextStyle(
          fontSize: 15.sp,
          color: Colors.black,
          height: 1.3,
  ...
  Widget _buildLotteryView(FastTableController controller) {
    return Column(
      children: [
        Container(
          padding: EdgeInsets.only(top: 20.w),
          child: _buildLotteryInfo(controller.fastTable.current),
        ),
        _buildLotteryBall(
          controller.fastTable.current,
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              _lotteryBall(
                controller.fastTable.last,
                font: Colors.white,
                color: const Color(0xff168c8c),
              ),
              _lotteryBall(
                controller.fastTable.before,
                font: Colors.white,
                color: const Color(0xff68ac7a),
              ),
              _lotteryBall(
                controller.fastTable.lastBefore,
  ...
  Widget _buildFastTable(FastTableController controller) {
    List<TableRow> rows = [];
    List<List<RenderCell>> renders = controller.fastTable.fastTable;
    ...
    return RepaintBoundary(
      child: Padding(
        ...
        child: Container(
          ...
          child: Table(
            defaultColumnWidth: FixedColumnWidth(26.3.w),
            children: rows,
          ),
        ),
      ),
