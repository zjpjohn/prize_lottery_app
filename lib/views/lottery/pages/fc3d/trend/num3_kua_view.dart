...
class Num3KuaView extends StatefulWidget {
  const Num3KuaView({
    super.key,
    required this.rows,
    required this.type,
  ...
  @override
  State<Num3KuaView> createState() => _Num3KuaViewState();
}

class _Num3KuaViewState extends State<Num3KuaView>
    with AutomaticKeepAliveClientMixin {
  @override
  Widget build(BuildContext context) {
    super.build(context);
    Get.put(LotteryKuaOmitController(widget.type));
    return Column(
      children: [
        SizedBox(
...
class KuaOmitView extends StatefulWidget {
  const KuaOmitView({
    super.key,
    required this.rows,
  ...
  final List<KuaOmit> omits;
  final N3KuaCensus census;

  @override
  State<KuaOmitView> createState() => _KuaOmitViewState();
}

class _KuaOmitViewState extends State<KuaOmitView> {
  ///
  ...
  Widget _buildLeftContent(List<KuaOmit> omits) {
    return SingleChildScrollView(
  ...
  List<Widget> _leftContentCells(List<KuaOmit> omits) {
    List<Widget> views = [];
  ...
  Widget _buildBodyContent(List<KuaOmit> omits) {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      physics: const NeverScrollableScrollPhysics(),
      ...
      child: SingleChildScrollView(
        ...
        child: SizedBox(
          width: cellWidth * 40 + left,
          height: cell * omits.length,
          child: CustomPaint(
            size: Size(cellWidth * 40 + left, cell * omits.length),
            foregroundPainter: RecDrawLinePainter(buildKuaPositions(omits)),
            child: Column(
  ...
  List<Widget> _bodyContentRows(List<KuaOmit> omits) {
    List<Widget> views = [];
  ...
  Widget _buildBodyRow(KuaOmit omit) {
    List<Widget> views = [
      _buildLottoCell(lotteries[omit.period]),
      ..._buildBaseOmit(omit.baseOmit.values),
      ..._buildAmpOmit(omit.ampOmit.values),
      ..._buildAmpAmpOmit(omit.ampAmp.values),
  ...
  List<Widget> _buildAmpOmit(List<OmitValue> values) {
    return values
        .map((e) => Container(
              width: cellWidth,
              height: cell,
  ...
  List<Widget> _buildAmpAmpOmit(List<OmitValue> values) {
    return values
        .map((e) => Container(
              width: cellWidth,
              height: cell,
  ...
  Widget _bottomBodyContent(N3KuaCensus census) {
    return SizedBox(
          ...
          child: Column(
            children: [
              ...
              _buildCensusRow(
                ...
                tail: census.ampFreq,
                ampAmp: census.ampAmpFreq,
                ott: census.ottFreq,
                bms: census.bmsFreq,
              ),
              _buildCensusRow(
                base: census.baseSeries,
                tail: census.ampSeries,
                ampAmp: census.ampAmpSeries,
  ...
  Widget _buildCensusRow({
    ...
    required List<OmitValue> ampAmp,
    ...
    views.addAll(ott
        .map((e) => _censusCell(value: e.value, color: greyBorder, width: 0.4))
        .toList());
