...
import 'package:prize_lottery_app/views/lottery/controller/lottery_state_controller.dart';
...
class Num3StateView extends StatefulWidget {
  const Num3StateView({
    super.key,
    required this.type,
    required this.rows,
  ...
  @override
  State<Num3StateView> createState() => _Num3StateViewState();
}

class _Num3StateViewState extends State<Num3StateView>
    with AutomaticKeepAliveClientMixin {
  @override
  Widget build(BuildContext context) {
    super.build(context);
    Get.put(LotteryStateController(widget.type));
    return Column(
      children: [
        SizedBox(
          height: widget.rows * trendCellSize + 0.5.w,
          child: RequestWidget<LotteryStateController>(
            builder: (controller) {
              return StateView(
                rows: widget.rows,
                censuses: controller.lottos,
              );
            },
          ),
        ),
        GetBuilder<LotteryStateController>(
          builder: (controller) {
...
class StateViewState extends State<StateView> {
  ...
  List<CensusCell> cells = [
    CensusCell(title: '开奖号码', width: 100.w, height: trendCellSize),
    CensusCell(title: '和值', width: 38.w, height: trendCellSize),
    CensusCell(title: '跨度', width: 38.w, height: trendCellSize),
    CensusCell(title: '历史同期', width: 100.w, height: trendCellSize),
    CensusCell(title: '历史遗漏', width: 100.w, height: trendCellSize),
    CensusCell(title: '大小形态', width: 100.w, height: trendCellSize),
    CensusCell(title: '大小比', width: 66.w, height: trendCellSize),
    CensusCell(title: '奇偶形态', width: 100.w, height: trendCellSize),
    CensusCell(title: '奇偶比', width: 66.w, height: trendCellSize),
    CensusCell(title: '012形态', width: 80.w, height: trendCellSize),
    CensusCell(title: '质合形态', width: 100.w, height: trendCellSize),
  ...
  List<Widget> _leftCells(List<LotteryCensus> censuses) {
    ...
    for (int i = 0; i < censuses.length; i++) {
      ...
      views.add(
        GestureDetector(
          onTap: () {
            Get.toNamed('/num3/follow/list/${omit.type}?period=${omit.period}');
          },
    ...
    views.add(
      _buildRowCell(
        child: Text(
          census.ott.ratio,
          style: TextStyle(
            color: census.ott.isSame ? redFont : blackFont,
            fontSize: 13.sp,
          ),
        ),
        index: 9,
