...
class Kl8CalculatorView extends StatelessWidget {
  ///
  ///
  const Kl8CalculatorView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        ...
        child: RequestWidget<Kl8CalculatorController>(
          builder: (controller) {
  ...
  ///
  /// 计算器面板
  Widget _buildCalcView(Kl8CalculatorController controller) {
    return Container(
      color: Colors.white,
      margin: EdgeInsets.only(bottom: 12.w),
  ...
  Widget _buildPickerView(Kl8CalculatorController controller) {
    return Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Text(
                          controller.awardCounter.playRule.name,
                          style: TextStyle(
                            color: const Color(0xFFFF0000),
                            fontSize: 14.sp,
                          ),
                  ...
                  child: Row(
                    children: [
                      ...
                      Padding(
                        padding: EdgeInsets.only(left: 12.w, top: 1.5.w),
                        child: Text(
                          '${controller.awardCounter.selected}',
                          style: TextStyle(
                            color: const Color(0xFFFF0000),
                            fontSize: 15.sp,
                          ),
  ...
  Widget _buildHitView(Kl8CalculatorController controller) {
  ...
  Widget _buildCalcBtn(Kl8CalculatorController controller) {
    return Container(
      margin: EdgeInsets.only(top: 10.w, bottom: 16.w),
      child: GestureDetector(
        onTap: () {
          controller.calcAwardCount();
        },
        child: Container(
          width: 200.w,
          height: 34.w,
  ...
  Widget _buildAwardCounter(Kl8CalculatorController controller) {
    return Column(
      children: [
        Container(
          alignment: Alignment.centerLeft,
          ...
          child: Text(
            '总计投注${controller.awardCounter.total}注，所需资金${controller.awardCounter.total * 2}元',
            style: TextStyle(
              fontSize: 14.sp,
        ...
        Container(
          ...
          child: Column(
            children: [
              _buildHeader(),
              ..._buildCounterList(controller.awardCounter),
            ],
          ),
        ),
        _buildBottomHint(),
  ...
  Widget _showRulePanel() {
    return GetBuilder<Kl8CalculatorController>(builder: (controller) {
              ...
              Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      rules[controller.awardCounter.ruleIdx]!.name,
                      style: TextStyle(
                        color: const Color(0xFFFF0000),
                        fontSize: 14.sp,
                      ),
                  ...
                  children: rules.values
                      .map(
                        (value) => RadioButton(
                          ...
                          selected:
                              controller.awardCounter.ruleIdx == value.size,
                          handle: (value) {
                            controller.playRule(int.parse(value));
                          },
                        ),
                      )
                      .toList(),
  ...
  Widget _showBasicPanel() {
    return GetBuilder<Kl8CalculatorController>(
      builder: (controller) {
        return ModalSheetView(
          title: '我的投注',
          height: Get.height * 9 / 16,
              ...
              Expanded(
                child: ScrollConfiguration(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Padding(
                      padding: EdgeInsets.only(top: 2.w, bottom: 24.w),
                      child: Wrap(
                        runSpacing: 10.w,
                        spacing: 7.w,
                        children: controller.awardCounter.playRule.limits
                            .map(
                              (e) => RadioButton(
                                text: '$e',
                                size: 42.w,
                                labelColor: Colors.red,
                                selected: controller.awardCounter.selected == e,
                                handle: (value) {
                                  controller.selectedBase(int.parse(value));
                                },
                              ),
                            )
                            .toList(),
                ...
                Padding(
                  ...
                  child: Wrap(
                    ...
                    children: List.generate(
                      controller.awardCounter.getTotalHit() + 1,
                      (index) => RadioButton(
                        text: '$index',
                        size: 42.w,
                        labelColor: Colors.red,
                        selected: controller.awardCounter.hit == index,
                        handle: (value) {
                          controller.selectedHit(int.parse(value));
                        },
                      ),
                    ),
                  ),
  ...
  List<Widget> _buildCounterList(AwardCounter counter) {
    List<Widget> rows = [];
    int size = counter.playRule.levels.length;
    List<int> levelList = counter.playRule.levels.keys.toList();
    for (int i = 0; i < size; i++) {
      rows.add(_buildCounter(
        level: counter.playRule.levels[levelList[i]]!,
        hit: counter.awardHits[levelList[i]],
        color: i % 2 == 1 ? const Color(0xFFF8F8F8) : Colors.white,
        radius: i == size - 1,
      ));
    }
    return rows;
  ...
  Widget _buildCounter({
    required AwardInfo level,
    required Color color,
    required bool radius,
    AwardHit? hit,
  }) {
    return Container(
      width: 336.w,
      ...
      child: Row(
        children: [
          ...
          _buildCounterCell(
            title: '${level.fix ? level.award : level.floats}',
            width: 84.w,
            right: true,
            color: color,
            bottom: !radius,
