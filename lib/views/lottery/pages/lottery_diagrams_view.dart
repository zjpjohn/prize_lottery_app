...
class LotteryDiagramsView extends StatelessWidget {
  const LotteryDiagramsView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        color: Colors.white,
        child: RequestWidget<DiagramsTableController>(
          builder: (controller) {
            return Stack(
              children: [
                ScrollConfiguration(
  ...
  Widget _buildPeriodView(DiagramsTableController controller) {
    return Container(
      width: 34.w,
  ...
  Widget _buildHeaderView(DiagramsTableController controller) {
    return Container(
      ...
      child: Column(
        children: [
          Text(
            '第${controller.diagramsTable.current.period}期${lotteryZhCns[controller.type]!}寻宝图',
            style: TextStyle(
              color: Colors.black,
              fontSize: 18.sp,
  ...
  Widget _buildTableDescription() {
    return Container(
      ...
      child: Text(
        '八卦图是专为选三型彩票提供的一种选号参考工具。首先在表中找到上期开奖号码，最佳位置就是三个号码的最直线或者三角排列；'
  ...
  Widget _buildLayoutContent(DiagramsTableController controller) {
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            _buildDiagramsTable(controller),
            _buildHelpView(),
          ],
        ),
      ),
  ...
  Widget _buildPosterWidget(DiagramsTableController controller) {
    return Container(
      color: Colors.white,
      width: Get.width,
      alignment: Alignment.center,
  ...
  Widget _buildDiagramsTable(DiagramsTableController controller) {
    List<TableRow> rows = [];
    List<List<RenderCell>> renders = controller.diagramsTable.diagramsTable;
    ...
    return RepaintBoundary(
      child: Padding(
        ...
        child: Container(
          ...
          child: Table(
            defaultColumnWidth: FixedColumnWidth(38.w),
            children: rows,
          ),
        ),
      ),
  ...
  Widget _buildLotteryView(DiagramsTableController controller) {
    return Column(
      children: [
        Container(
          padding: EdgeInsets.only(top: 20.w),
          child: _buildLotteryInfo(controller.diagramsTable.current),
        ),
        _buildLotteryBall(
          controller.diagramsTable.current,
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              _lotteryBall(
                controller.diagramsTable.last,
                font: Colors.white,
                color: const Color(0xff168c8c),
              ),
              _lotteryBall(
                controller.diagramsTable.before,
                font: Colors.white,
                color: const Color(0xff68ac7a),
              ),
              _lotteryBall(
                controller.diagramsTable.lastBefore,
