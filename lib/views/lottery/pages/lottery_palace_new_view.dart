...
class LotteryPalaceNewView extends StatelessWidget {
  const LotteryPalaceNewView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      right: GestureDetector(
        onTap: () {
          Get.toNamed('/palace/${Get.parameters['type'] ?? 'fc3d'}');
        },
      ...
      content: Container(
        color: Colors.white,
        child: RequestWidget<LotteryPalaceNewController>(
          builder: (controller) {
            return Stack(
              children: [
                ScrollConfiguration(
  ...
  Widget _buildPeriodView(LotteryPalaceNewController controller) {
    return Container(
      width: 34.w,
  ...
  Widget _buildLayoutContent(LotteryPalaceNewController controller) {
  ...
  Widget _buildPosterWidget(LotteryPalaceNewController controller) {
    return Container(
      color: Colors.white,
      width: Get.width,
  ...
  Widget _buildTableView(LotteryPalaceNewController controller) {
    List<Widget> views = [];
  ...
  double _calcTableHeight(LotteryPalaceNewController controller) {
    return (controller.cells.length - 1) * (34.w - 14.2.w * tan(pi / 6)) + 34.w;
  }

  Widget _buildRows(int index, List<Cell> rows) {
    double left = index % 2 == 0 ? 0 : 14.2.w;
    double top = index * (34.w - 14.2.w * tan(pi / 6));
    return Positioned(
      left: left,
      top: top,
      child: Row(
  ...
  Widget _buildHeaderView(LotteryPalaceNewController controller) {
    return Container(
  ...
  Widget _buildLotteryView(LotteryPalaceNewController controller) {
    return Column(
      children: [
        Container(
