...
class SsqRedTrendView extends StatefulWidget {
  ///
  ///
  const SsqRedTrendView({
    super.key,
    required this.rows,
    required this.omits,
  ...
  final SsqOmitCensus census;

  @override
  SsqRedTrendViewState createState() => SsqRedTrendViewState();
}

class SsqRedTrendViewState extends State<SsqRedTrendView>
      ...
      (index) => Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            ...
            bottom: BorderSide(color: greyBorder, width: 0.4),
            right: BorderSide(
              color: index == 16 ? redFont : greyBorder,
              width: index == 16 ? 1 : 0.4,
            ),
          ),
        ),
        child: Text(
  ...
  Widget bodyRow(LotteryOmit omit) {
    return SingleChildScrollView(
      ...
      scrollDirection: Axis.horizontal,
      child: Row(
        children: _buildBodyCells(omit.red!.values),
      ),
    );
  }
        ...
        .map(
          (e) => Container(
            ...
            decoration: BoxDecoration(
              border: Border(
                right: BorderSide(
                  color: e.key == '17' ? redFont : greyBorder,
                  width: e.key == '17' ? 1 : 0.4,
                ),
                bottom: BorderSide(color: greyBorder, width: 0.4),
              ),
            ),
  ...
  Widget _bottomBodyContent(SsqOmitCensus census) {
    return SizedBox(
      width: MediaQuery.of(context).size.width - left,
      child: SingleChildScrollView(
        ...
        child: SizedBox(
          ...
          child: Column(
            children: [
              _buildCensusRow(census.redMax),
              _buildCensusRow(census.redAvg),
              _buildCensusRow(census.redFreq),
              _buildCensusRow(census.redSeries),
            ],
          ),
        ),
      ),
