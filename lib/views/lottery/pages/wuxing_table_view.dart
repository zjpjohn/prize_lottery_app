...
class WuXingTableView extends StatelessWidget {
  const WuXingTableView({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutContainer(
      ...
      content: Container(
        color: Colors.white,
        child: RequestWidget<WuXingTableController>(
          builder: (controller) {
            return Stack(
              children: [
                ScrollConfiguration(
  ...
  Widget _buildHeaderView(WuXingTableController controller) {
    return Container(
      ...
      child: Column(
        children: [
          Text(
            '第${controller.wuXing.current.period}期${lotteryZhCns[controller.type]!}五行图',
            style: TextStyle(
              color: Colors.black,
              fontSize: 18.sp,
  ...
  Widget _buildTableDescription() {
    return Container(
      ...
      child: Text(
        '五行图是专为选三型彩票提供的一种选号参考工具。首先在表中找到上期开奖号码，最佳位置就是三个号码的最直线或者三角排列；'
        '然后结合本期试机号进行精细定位，重点考虑周围的号码进行组号。本表可根据自身选号经验，探索更多灵活使用技巧。'
        '五行图每天下午17:00左右试机号更新完成后会显示最新一期的数据。用户可长按选号图表，将本图表保存至本地更方便地查看。',
        style: TextStyle(
          fontSize: 15.sp,
          color: Colors.black,
          height: 1.3,
  ...
  Widget _buildPeriodView(WuXingTableController controller) {
    return Container(
      width: 34.w,
  ...
  Widget _buildLayoutContent(WuXingTableController controller) {
  ...
  Widget _buildPosterWidget(WuXingTableController controller) {
    return Container(
      color: Colors.white,
      width: Get.width,
      alignment: Alignment.center,
  ...
  Widget _buildLotteryView(WuXingTableController controller) {
    return Column(
      children: [
        Container(
          padding: EdgeInsets.only(top: 20.w),
          child: _buildLotteryInfo(controller.wuXing.current),
        ),
        _buildLotteryBall(
          controller.wuXing.current,
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              _lotteryBall(
                controller.wuXing.last,
                font: Colors.white,
                color: const Color(0xff168c8c),
              ),
              _lotteryBall(
                controller.wuXing.before,
                font: Colors.white,
                color: const Color(0xff68ac7a),
              ),
              _lotteryBall(
                controller.wuXing.lastBefore,
  ...
  Widget _buildWuXingTable(WuXingTableController controller) {
    List<TableRow> rows = [];
    List<List<RenderCell>> renders = controller.wuXing.wuXingTable;
