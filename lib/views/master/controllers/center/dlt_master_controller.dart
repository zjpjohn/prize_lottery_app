...
class DltMasterController extends AbsPageQueryController {
  ///
  ///方案更新专家信息
  ...
  ///
  List<TabEntry> dropEntries = dltChannels.entries
  ...
  List<Future<void>> getRequestFutures() {
    Future<void> renewAsync = LottoMasterRepository.dltRenewMaster()
        .then((value) => renewMasters = value);
    Future<void> recentBrowseAsync = UserInfoRepository.recentBrowse('dlt')
        .then((value) => browseRecord = value);
    Future<void> schemaAsync =
        LottoMasterRepository.dltSchemaMasters().then((value) => schemas
          ..clear()
          ..addAll(value));
    Future<void> rankAsync =
        MasterRankRepository.mulDltMasterRanks(page: 1, limit: 5)
            .then((value) => ranks
              ..clear()
              ..addAll(value.records));
    Future<void> homeAsync =
        LottoMasterRepository.homeMasters('dlt').then((value) => homeMasters
          ..clear()
          ..addAll(value));
    return [
      renewAsync,
