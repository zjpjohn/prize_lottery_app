...
class SsqMasterController extends AbsPageQueryController {
  ///
  ///方案更新专家信息
  ...
  ///
  List<TabEntry> dropEntries = ssqChannels.entries
  ...
  List<Future<void>> getRequestFutures() {
    Future<void> renewAsync = LottoMasterRepository.ssqRenewMaster()
        .then((value) => renewMasters = value);
    Future<void> recentBrowseAsync = UserInfoRepository.recentBrowse('ssq')
        .then((value) => browseRecord = value);
    Future<void> schemaAsync =
        LottoMasterRepository.ssqSchemaMasters().then((value) => schemas
          ..clear()
          ..addAll(value));
    Future<void> rankAsync =
        MasterRankRepository.mulSsqMasterRanks(page: 1, limit: 5)
            .then((value) => ranks
              ..clear()
              ..addAll(value.records));
    Future<void> homeAsync =
        LottoMasterRepository.homeMasters('ssq').then((value) => homeMasters
          ..clear()
          ..addAll(value));
    return [
      renewAsync,
