...
class SsqMasterDetailController extends AbsRequestController {
  ///
  ///专家标识
  late String masterId;

  ///专家信息
  late SsqMasterDetail master;
  ...
  ///
  /// 推荐专家信息
  List<SsqMasterMulRank> masters = [];

  ///历史预测
  List<SsqHistory> histories = [];
  ...
  List<TabEntry> tabEntries = [
    ...
    TabEntry(key: 'r12', value: '红12码'),
    TabEntry(key: 'r20', value: '红20码'),
    TabEntry(key: 'r25', value: '红25码'),
    TabEntry(key: 'rk3', value: '红杀三'),
    TabEntry(key: 'rk6', value: '红杀六'),
    TabEntry(key: 'b3', value: '蓝三码'),
    TabEntry(key: 'b5', value: '蓝五码'),
    TabEntry(key: 'bk', value: '篮杀码'),
  ];
  ...
  void showRecommendMasters(Function() callback) {
    ...
    update();
    MasterRankRepository.mulSsqMasterRanks(page: 1, limit: 7).then((value) {
  ...
  void calcHistory(List<SsqHistory> datas) {
    record = HistoryRecord()
      ...
      ..addRecord(
          ...
          datas
              .map((e) =>
                  HitItem.parseItem(e.period, false, e.red25, e.reds, e.blues))
              .toList())
      ..addRecord(
          'rk3',
          datas
      ...
      ..addRecord(
          ...
          datas
              .map((e) =>
                  HitItem.parseItem(e.period, true, e.blue3, e.reds, e.blues))
              .toList())
      ..addRecord(
          'b5',
          datas
              .map((e) =>
                  HitItem.parseItem(e.period, true, e.blue5, e.reds, e.blues))
              .toList())
      ..addRecord(
          'bk',
          datas
              .map((e) => HitItem.parseItem(
                  e.period, true, e.blueKill, e.reds, e.blues))
              .toList());
  }
  ...
  Future<void> request() async {
    ...
    Future<void> masterAsync = LottoMasterRepository.ssqMasterDetail(masterId)
        .then((value) => master = value);

    Future<void> historyAsync =
        ForecastInfoRepository.ssqHistories(masterId).then((value) {
      histories
        ..clear()
        ..addAll(value);
