...
import 'package:prize_lottery_app/views/master/controllers/detail/dlt_master_detail_controller.dart';
...
class DltMasterDetailView extends StatefulWidget {
  const DltMasterDetailView({super.key});

  @override
  State<DltMasterDetailView> createState() => _DltMasterDetailViewState();
}

class _DltMasterDetailViewState extends State<DltMasterDetailView> {
  ///
  ///
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          top: true,
          child: RequestWidget<DltMasterDetailController>(
            global: false,
            duration: 50,
            init: DltMasterDetailController(),
            builder: (controller) {
                  ...
                  return [
                    SliverPersistentHeader(
                      ...
                      delegate: MasterHeader(
                          ...
                          onTap: () {
                            Get.toNamed(
                              '/dlt/master/feature/${controller.master.master.masterId}',
                            );
                          }),
                    ),
  ...
  Widget buildMasterView(DltMasterDetailController controller) {
    DltMasterDetail detail = controller.master;
    return SizedBox(
      width: Get.width,
      child: Column(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  SizedBox(width: 16.w),
                  if (detail.red3 != null)
                    AchieveMark(name: '三胆', achieve: detail.red3!.count),
                  if (detail.red20 != null)
                    AchieveMark(name: '围红', achieve: detail.red20!.count),
                  if (detail.redKill != null)
                    AchieveMark(name: '杀红', achieve: detail.redKill!.count),
                ],
              ),
            ),
          ),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      Get.toNamed('/dlt/forecast/${detail.master.masterId}');
                    },
                    child: Container(
                      height: 32.w,
                      alignment: Alignment.center,
  ...
  List<String> getSkillTxt(DltMasterDetail detail) {
    List<String> txt = [];
    if (detail.red20 != null && detail.red20!.rate >= 0.5) {
      txt.add('围20码');
    }
    if (detail.redKill != null && detail.redKill!.rate >= 0.65) {
      txt.add('杀红三');
    }
    if (detail.blueKill != null && detail.blueKill!.rate >= 0.8) {
      txt.add('杀蓝五');
    }
    if (txt.isEmpty) {
      txt.add('加油ing');
  ...
  Widget _buildRecommendMasters(DltMasterDetailController controller) {
    return Container(
              ...
              child: Row(
                children: [
                  ...
                  ...controller.masters.map(
                    (e) => GestureDetector(
                      onTap: () {
                        Get.toNamed('/dlt/master/${e.master.masterId}');
                      },
                      behavior: HitTestBehavior.opaque,
                      child: Container(
