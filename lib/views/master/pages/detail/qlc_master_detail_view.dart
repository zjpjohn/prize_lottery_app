...
import 'package:prize_lottery_app/views/master/controllers/detail/qlc_master_detail_controller.dart';
...
class QlcMasterDetailView extends StatefulWidget {
  const QlcMasterDetailView({super.key});

  @override
  State<QlcMasterDetailView> createState() => _QlcMasterDetailViewState();
}

class _QlcMasterDetailViewState extends State<QlcMasterDetailView> {
  ///
  ///
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          top: true,
          child: RequestWidget<QlcMasterDetailController>(
            global: false,
            duration: 50,
            init: QlcMasterDetailController(),
            builder: (controller) {
                  ...
                  return [
                    SliverPersistentHeader(
                      ...
                      delegate: MasterHeader(
                        ...
                        onTap: () {
                          Get.toNamed(
                            '/qlc/master/feature/${controller.master.master.masterId}',
                          );
                        },
                      ),
                    ),
  ...
  Widget buildMasterView(QlcMasterDetailController controller) {
    QlcMasterDetail detail = controller.master;
    return SizedBox(
      width: Get.width,
      child: Column(
          ...
          Container(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  SizedBox(width: 16.w),
                  if (detail.red22 != null)
                    AchieveMark(name: '围码', achieve: detail.red22!.count),
                  if (detail.kill3 != null)
                    AchieveMark(name: '杀码', achieve: detail.kill3!.count),
                ],
              ),
            ),
          ),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      Get.toNamed('/qlc/forecast/${detail.master.masterId}');
                    },
                    child: Container(
                      height: 32.w,
                      alignment: Alignment.center,
  ...
  List<String> getSkillTxt(QlcMasterDetail detail) {
    List<String> txt = [];
    if (detail.red22 != null && detail.red22!.rate >= 0.6) {
      txt.add('围22码');
    }
    if (detail.kill3 != null && detail.kill3!.rate >= 0.65) {
      txt.add('杀三码');
    }
    if (txt.isEmpty) {
      txt.add('加油ing');
  ...
  Widget _buildRecommendMasters(QlcMasterDetailController controller) {
    return Container(
