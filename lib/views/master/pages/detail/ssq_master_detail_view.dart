...
import 'package:prize_lottery_app/views/master/controllers/detail/ssq_master_detail_controller.dart';
...
class SsqMasterDetailView extends StatefulWidget {
  const SsqMasterDetailView({super.key});

  @override
  State<SsqMasterDetailView> createState() => _SsqMasterDetailViewState();
}

class _SsqMasterDetailViewState extends State<SsqMasterDetailView> {
  ///
  ///
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          top: true,
          child: RequestWidget<SsqMasterDetailController>(
            global: false,
            duration: 50,
            init: SsqMasterDetailController(),
            builder: (controller) {
                  ...
                  return [
                    SliverPersistentHeader(
                      ...
                      delegate: MasterHeader(
                        ...
                        onTap: () {
                          Get.toNamed(
                            '/ssq/master/feature/${controller.master.master.masterId}',
                          );
                        },
                      ),
                    ),
  ...
  Widget buildMasterView(SsqMasterDetailController controller) {
    SsqMasterDetail detail = controller.master;
    return SizedBox(
      width: Get.width,
      child: Column(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  ...
                  if (detail.red25 != null)
                    AchieveMark(name: '围红', achieve: detail.red25!.count),
                  if (detail.rk3 != null)
                    AchieveMark(name: '杀红', achieve: detail.rk3!.count),
                ],
              ),
            ),
          ),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      Get.toNamed('/ssq/forecast/${detail.master.masterId}');
                    },
                    child: Container(
                      height: 32.w,
                      alignment: Alignment.center,
  ...
  List<String> getSkillTxt(SsqMasterDetail detail) {
    List<String> txt = [];
    if (detail.red25 != null && detail.red25!.rate >= 0.55) {
      txt.add('围25码');
    }
    if (detail.rk3 != null && detail.rk3!.rate >= 0.65) {
      txt.add('杀红三');
    }
    if (detail.bk != null && detail.bk!.rate >= 0.8) {
      txt.add('杀蓝五');
    }
    if (txt.isEmpty) {
      txt.add('加油ing');
  ...
  Widget _buildRecommendMasters(SsqMasterDetailController controller) {
    return Container(
