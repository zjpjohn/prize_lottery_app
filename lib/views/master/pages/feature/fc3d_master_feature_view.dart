...
class Fc3dMasterFeatureView extends StatelessWidget {
  ///
  const Fc3dMasterFeatureView({super.key});

  @override
  Widget build(BuildContext context) {
    return LayoutContainer(
      title: '专家画像',
      content: RequestWidget<Fc3dMasterFeatureController>(
        builder: (controller) => _buildMasterFeature(controller),
      ),
    );
  }

  Widget _buildMasterFeature(Fc3dMasterFeatureController controller) {
  ...
  Widget _buildRateFeature(Fc3dMasterRate rate) {
    return Container(
  ...
  Widget _buildHitFeature(Fc3dMasterFeatureController controller) {
    return Column(
      children: [
        Container(
          alignment: Alignment.centerLeft,
  ...
  Widget _buildHitsView(List<Fc3dICaiHit> hits) {
    List<Widget> items = [];
    for (int i = 0; i < hits.length; i++) {
      Fc3dICaiHit hit = hits[i];
      items.add(
        Row(
          children: [
