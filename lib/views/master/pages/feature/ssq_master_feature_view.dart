...
class SsqMasterFeatureView extends StatelessWidget {
  ///
  ///
  const SsqMasterFeatureView({super.key});

  @override
  Widget build(BuildContext context) {
    return LayoutContainer(
      title: '专家画像',
      content: RequestWidget<SsqMasterFeatureController>(
        builder: (controller) => _buildMasterFeature(controller),
      ),
    );
  }

  Widget _buildMasterFeature(SsqMasterFeatureController controller) {
  ...
  Widget _buildRateFeature(SsqMasterRate rate) {
    return Container(
      ...
      child: Column(
        children: [
          ...
          RateFeatureChart(
            title: '红球三胆',
            level: rate.red3Level(),
            delta: rate.red3Delta(),
            rate: rate.red3,
          ...
          RateFeatureChart(
            title: '红球20码',
            level: rate.red20Level(),
            delta: rate.red20Delta(),
            rate: rate.red20,
          ...
          RateFeatureChart(
            title: '红球25码',
            level: rate.red25Level(),
            delta: rate.red25Delta(),
            rate: rate.red25,
            average: rate.red25Avg,
            bottom: 4.w,
            width: 230,
          ),
          RateFeatureChart(
          ...
          RateFeatureChart(
            title: '蓝球五码',
            level: rate.blue5Level(),
            delta: rate.blue5Delta(),
            rate: rate.blue5,
            average: rate.blue5Avg,
            bottom: 4.w,
            width: 230,
          ),
          RateFeatureChart(
  ...
  Widget _buildHitFeature(SsqMasterFeatureController controller) {
    return Column(
      children: [
        Container(
          alignment: Alignment.centerLeft,
  ...
  Widget _buildHitHeader() {
    return Row(
      children: [
        ...
        _cell(title: '25码', color: Colors.black, fontSize: 12),
        _cell(title: '杀红', color: Colors.black, fontSize: 12),
        _cell(title: '蓝五', color: Colors.black, fontSize: 12),
        _cell(title: '杀蓝', color: Colors.black, right: false, fontSize: 12),
      ],
    );
  }

  Widget _buildHitsView(List<SsqICaiHit> hits) {
    List<Widget> items = [];
    for (int i = 0; i < hits.length; i++) {
      SsqICaiHit hit = hits[i];
      items.add(
        Row(
          children: [
            ...
            _hitCell(pair: hit.r25Hit(), bottom: i < hits.length - 1),
            _hitCell(pair: hit.rk3Hit(), bottom: i < hits.length - 1),
            _hitCell(pair: hit.b5Hit(), bottom: i < hits.length - 1),
            _hitCell(
              pair: hit.bkHit(),
              right: false,
