...
class DltItemRankView extends StatefulWidget {
  ///
  const DltItemRankView({super.key});

  @override
  DltItemRankViewState createState() => DltItemRankViewState();
}

class DltItemRankViewState extends State<DltItemRankView>
  ...
  ///
  List<TabEntry> tabEntries = dltChannels.entries
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            RefreshWidget<DltItemRankController>(
            ...
            StreamBuilder<bool>(
              ...
              builder: (context, snapshot) {
                ...
                return Container(
                  ...
                  child: GetBuilder<DltItemRankController>(
                    builder: (controller) {
  ...
  Widget _buildRankContainer(DltItemRankController controller) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        DltMasterRank rank = controller.ranks[realIndex];
  ...
  Widget _topRankMasterView(
    DltItemRankController controller,
    List<DltMasterGlad> glads,
    List<DltMasterRank> ranks,
  ) {
    return Stack(
      children: [
  ...
  List<Widget> _topRankMasters(List<DltMasterRank> ranks) {
    List<Widget> views = [];
    for (int i = 0; i < ranks.length; i++) {
      views.add(ItemRankEntryWidget(
        ...
        onTap: () {
          String channel = tabEntries[_tabController.index].key;
          Get.toNamed(
            '/dlt/master/${ranks[i].master.masterId}',
            parameters: {'channel': channel},
          );
        },
      ));
  ...
  Widget _headerBackground(List<DltMasterGlad> glads) {
    return Stack(
      ...
      children: [
        CachedAvatar(
          ...
          url: ResourceStore().resource(R.dltItemRankBg),
        ),
        VerticalMarquee(
          height: 28.w,
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (_tabController.index == _tabController.animation!.value) {
        DltItemRankController controller = Get.find<DltItemRankController>();
