...
class Fc3dWarningController extends AbsRequestController {
  ///预警数据
  ...
  set period(String? period) {
    ...
    if (results[_period] == null) {
      ...
      WarnRecommendRepository.fc3dWarnRecommend(period: _period).then((value) {
        recommend = value;
        update();
  ...
  List<Future<void>> asyncTasks() {
    return [
      WarnRecommendRepository.fc3dWarnRecommend().then((value) {
        recommend = value;
      ...
      WarnRecommendRepository.fc3dPeriods().then((value) {
        periods = value;
        if (periods.isNotEmpty) {
          _period = periods[0];
        }
